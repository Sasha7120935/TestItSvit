...
@if(count($beer_producers) > 0)
    <div class="card">
        <div class="card-heading">
            Types of beer
        </div>
        <div class="card-body">
            <table class="table.table.table-striped beer_producer-table">
                <thead>
                <th>beer</th>
                <th>&nbsp;</th>
                </thead>
                <tbdoy>
                    @foreach($beer_producers as $beer_producer)

                        <td class="table-text">
                            <div>{{$beer_producer -> name}}</div>
                        </td>
                        <td>
                            <form action="{{url('beer_producer/'.$beer_producer->id)}}" method="POST">
                                {{csrf_field()}}
                                {{method_field('DELETE')}}
